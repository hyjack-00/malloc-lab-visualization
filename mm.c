...
static void *find_fit(size_t asize)
{
    ...
    for (void *p = heap_listp; GET_SIZE(HDRP(p)) > 0; p = NEXT_BLKP(p)){
        size_t blk_alloc = GET_ALLOC(HDRP(p));
        size_t blk_size = GET_SIZE(HDRP(p));
        if (!blk_alloc && (asize <= blk_size)) {
            if (best_size > blk_size) {
                best_size = blk_size;
                best_bp = p;
            }
        }

...
/*
  ...
  The new code added by "https://github.com/hyjack-00" are marked with 
    #ifdef HEAP_VISUAL
      ...
    #endif
  to clearly differentiate the new code from the original. 
  
  The code under #ifdef HEAP_VISUAL is copyright Yujie Huang, 2023,
  liscenced under the MIT License.

  It includes functions for displaying heap status and blocks positions,
  added for research and educational purposes. Detailed information could 
  be found in README-heapvisual.

  The remaining code is used with permission and retains the original 
  copyright and license restrictions as indicated by the original authors.
*/
#define HEAP_VISUAL
#ifdef HEAP_VISUAL
    #include "heapvisual.h"
    const char *heap_log_file = "heap.log";
    FILE *fp_log;
#endif
...
int main(int argc, char **argv)
{
    ...
    #ifdef HEAP_VISUAL
        fp_log = fopen(heap_log_file, "w");
        if (fp_log == NULL) {
            perror("ERROR. Couldn't open heap log file");
            return 1;
        }
    #endif
    ...
    for (i=0; i < num_tracefiles; i++) {
        trace = read_trace(tracedir, tracefiles[i]);
        #ifdef HEAP_VISUAL
            log_trace_init(fp_log, tracefiles[i], mem_heap_lo());
        #endif
